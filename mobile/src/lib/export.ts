...
import { averageRating, averageRatingPerQuestion, recommendationBreakdown, monthlyTrend } from './analytics';
...
export function exportPPT(items: Feedback[], counsellor?: Counsellor) {
  const ppt = new PptxGenJS();
  ppt.layout = 'LAYOUT_16x9';
  const title = counsellor ? `${counsellor.name} Feedback Report` : 'Wellness Centre Feedback Report';

  const slide1 = ppt.addSlide();
  slide1.background = { color: 'F9FAFB' };
  slide1.addText(title, { x: 1, y: 2, w: 8, h: 1, fontSize: 28, color: '6B46C1', bold: true });
  slide1.addText('IIT Madras Wellness Centre Feedback Platform', { x: 1, y: 3.2, w: 8, h: 0.5, fontSize: 14, color: '4B5563' });
  slide1.addText(`Generated on ${new Date().toLocaleDateString()}`, { x: 1, y: 3.8, w: 8, h: 0.5, fontSize: 12, color: '9CA3AF' });

  const avg = averageRating(items);
  const rec = recommendationBreakdown(items);
  const slide2 = ppt.addSlide();
  slide2.background = { color: 'FFFFFF' };
  slide2.addText('Summary Metrics', { x: 0.5, y: 0.5, w: 9, h: 0.6, fontSize: 20, color: '6B46C1', bold: true });
  ...
  slide3.addText('Ratings by Question', { x: 0.5, y: 0.3, w: 9, h: 0.6, fontSize: 18, color: '6B46C1', bold: true });
  slide3.addChart(ppt.ChartType.bar, chartData, { x: 0.5, y: 1, w: 9, h: 4.5, barDir: 'bar', chartColors: ['6B46C1'] });

  const trend = monthlyTrend(items);
  if (trend.length > 0) {
    const trendData = [{ name: 'Monthly Avg', labels: trend.map((t) => t.month), values: trend.map((t) => Number(t.avg.toFixed(2))) }];
    const slide4 = ppt.addSlide();
    slide4.addText('Monthly Trend', { x: 0.5, y: 0.3, w: 9, h: 0.6, fontSize: 18, color: '6B46C1', bold: true });
    slide4.addChart(ppt.ChartType.line, trendData, { x: 0.5, y: 1, w: 9, h: 4.5, chartColors: ['10B981'] });
  }

  const comments = items.filter((i) => i.comments).slice(0, 6);
  if (comments.length) {
    const slide5 = ppt.addSlide();
    slide5.addText('Selected Comments', { x: 0.5, y: 0.3, w: 9, h: 0.6, fontSize: 18, color: '6B46C1', bold: true });
    let y = 1;
  ...
  ppt.writeFile({ fileName: fileName(counsellor ? counsellor.name.replace(/\s+/g, '_') : 'Institution', 'pptx') });
}
...
export function exportPDF(items: Feedback[], counsellor?: Counsellor) {
  ...
  (doc as any).autoTable({
    ...
    body: rows,
    theme: 'grid',
    headStyles: { fillColor: [107, 70, 193], textColor: 255 },
  });
...
export function exportExcel(items: Feedback[], counsellor?: Counsellor) {
  ...
  const header = ['Date', 'Counsellor', ...FEEDBACK_QUESTIONS.map((q) => q.label), 'Recommendation', 'Anonymous', 'Comments'];
  const rows = items.map((i) => [
    new Date(i.submitted_at).toLocaleDateString(),
    counsellor ? counsellor.name : i.counsellor_id,
  ...
  XLSX.writeFile(wb, fileName(counsellor ? counsellor.name.replace(/\s+/g, '_') : 'Institution', 'xlsx'));
}
