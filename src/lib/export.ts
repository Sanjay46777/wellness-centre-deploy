...
export function exportPPT(items: Feedback[], counsellor?: Counsellor) {
  ...
  slide2.addText('Summary Metrics', { x: 0.5, y: 0.5, w: 9, h: 0.6, fontSize: 20, color: '8B1E1E', bold: true });
  ...
  slide3.addText('Ratings by Question', { x: 0.5, y: 0.3, w: 9, h: 0.6, fontSize: 18, color: '8B1E1E', bold: true });
  slide3.addChart(ppt.ChartType.bar, chartData, { x: 0.5, y: 1, w: 9, h: 4.5, barDir: 'bar', chartColors: ['8B1E1E'] });
  ...
  if (trend.length > 0) {
    ...
    slide4.addText('Monthly Trend', { x: 0.5, y: 0.3, w: 9, h: 0.6, fontSize: 18, color: '8B1E1E', bold: true });
  ...
  if (comments.length) {
    ...
    slide5.addText('Selected Comments', { x: 0.5, y: 0.3, w: 9, h: 0.6, fontSize: 18, color: '8B1E1E', bold: true });
    let y = 1;
...
export function exportExcel(items: Feedback[], counsellor?: Counsellor) {
  ...
  const rows = items.map((i) => [
    ...
    ...FEEDBACK_QUESTIONS.map((q) => i[q.key as keyof Feedback] ?? ''),
    i.recommendation ?? '',
  ...
  triggerDownload(new Blob([buf], { type: 'application/octet-stream' }), fileName(counsellor ? counsellor.name.replace(/\s+/g, '_') : 'Institution', 'xlsx'));
}
